...
# -- Subdataset with e65
LAC_e50_countries <- UN_WPP_LT_LAC %>% 
  #filter(Location!="Latin America and the Caribbean") %>% 
  filter(AgeGrpStart==50) %>% 
...
LAC_e50_leaders <- LAC_e50_countries %>% 
...
LAC_e50_leaders$sexo <- factor(LAC_e50_leaders$sexo, 
                               levels = c(1,2),
                               labels = c("Hombres", "Mujeres"))



Figure_A5 <- ggplot(LAC_e50_leaders, mapping = aes(x=Year, y=ex,
                                                   color=Pais)) +
  ...
  scale_color_manual(values = Color_figA3) +
  theme_bw() +
  theme_minimal( )+
  ...
  labs(#title ="A) Latin American countries (n=22)",
    ...
    y=bquote("Esperanza de vida " ~e[50]~ " "),
    x="Año")
Figure_A5
ggsave(filename = "Figure A5.png",
...
Mex_e50 <- LAC_e50_leaders %>% 
  filter(Location=="Mexico") %>% 
  mutate(sexo = case_when(Sex=="Male"~ 1,
                          Sex=="Female" ~ 2),
         order = rank)  

Mex_e50$sexo <- factor(Mex_e50$sexo, 
                       levels = c(1,2),
                       labels = c("Hombres", "Mujeres"))



Figure_A6 <- ggplot(Mex_e50, mapping = aes(x=Year, y=rank)) +
  geom_line(size=1.2) +
  facet_grid(.~sexo) +
  scale_x_continuous(breaks = c(seq(1990,2020,5))) +
  ...
  labs(#title ="A) Latin American countries (n=22)",
    ...
    color = "Estimate",
    #caption="Source data: ONS Longitudinal Study, Statistics Finland & Turin Longitudinal Study",
    y=bquote("Posición del ranking en " ~e[50]~ ""),
    x="Año")
Figure_A6
ggsave(filename = "Figure A6.png",
       path= "Figures/Appendix/",
       dpi = 320, width = 8, height = 6,
       bg = "transparent")
